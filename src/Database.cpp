...
namespace Engine {
    ...
    public:
        SQLiteDatabase(std::string database) {
            if (!hasInited) {
        ...
        bool Execute(std::string statement) override {
            char* errmsg = 0;
        ...
        std::vector< std::map<std::string, std::string> > ExecutePrepare(std::string statement) override {
